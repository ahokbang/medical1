 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "### 아래 정보를 DataFrame으로 출력하시오.\n",
    "### 1. 컬럼 : no,id,pw,name,phone,email,address,gender,major,grade,hobby,ip_address"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    "# 이름으로 정렬, default는 순차정렬, 순차정렬 : ascending = True, 역순정렬: ascending=False\n",
    "df.sort_index(ascending=False, inplace=True) # 고정시키려면,  inplace=True\n",
    "df #  inplace=True 없으면 위에서 역순정렬 한 게 여기서는 의미없어. 순차정렬로 출력됨."
   ]
  }
 ],
 "metadata": {
