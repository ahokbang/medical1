...
def stu_insert():
    while True :
            ...
            cnt += 1 # 학번 증가
            print("입력 데이터 : ", student) # lsit -> dict
            print(students) 
...
# 학생성적검색 함수
def stu_search():
    # 학생검색 -> 이름으로 검색을 해서 해당하는 학생이 있으면 해당학생 성적 출력
    # 1명 출력
    search_students = []
    print("[ 학생성적 검색 ]")
...
while True:
    ...
    if choice == 1 :
        ...
        stu_file.stu_save(students) # 학생성적파일 저장함수 호출
        
    # 2. 학생성적전체출력
    elif choice == 2 :
        stu_grade_print(students) # 학생성적출력함수 호출
